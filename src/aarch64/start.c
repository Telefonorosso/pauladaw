...
void secondary_boot(void)
{
    ...
    if (cpu_id == 1)
    {
        if (async_log) {
            ...
            /* Paula USB: CPU1 runs validated CACHE-FIX2 renderer. */
            extern void emu68_usb_audio_cpu1_worker(void);
            emu68_usb_audio_cpu1_worker();
        }
#endif
    }
    else if (cpu_id == 2)
...
#if defined(PISTORM_CLASSIC)
    else if (cpu_id == 3 && !ppc_enable)
    {
        /* Paula USB: preserve known-good sole ownership of DWC2 on CPU3. */
        extern void emu68_usb_audio_cpu3_worker(void);
        emu68_usb_audio_cpu3_worker();
    }
#endif
#else
    (void)async_log;
