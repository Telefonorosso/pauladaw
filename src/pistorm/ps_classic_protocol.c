...
#include "cache.h"
#include "intc.h"
#include "../raspi/paula/paula_mirror.h"
...
//        return;
    ...
    /* The actual bus operation has completed. Odd words are already split
     * into byte transactions above, so do not emit a duplicate word event. */
    if (!(address & 1u)) {
        paula_mirror_capture(address, data, 2u);
        paula_mirror_invalidate(address, 2u);
    }
...
//        return;
    ...
    const unsigned int original_byte = data & 0xffu;
    ...
    *(gpio + 2) = LE32(INPUT[2]);
    ...
    while (*(gpio + 13) & LE32((1 << PIN_TXN_IN_PROGRESS))) {}
    paula_mirror_capture(address, original_byte, 1u);
    paula_mirror_invalidate(address, 1u);
...
extern void emu68_usb_audio_housekeeper_poll(void);
...
void ps_housekeeper() 
{
    ...
    /* Service USB UVC at approximately 1 kHz from the housekeeper core. */
    const uint64_t uvc_poll_ticks = (tmp >= 1000) ? (tmp / 1000) : 1;
    uint64_t uvc_last_poll;
    asm volatile("mrs %0, CNTPCT_EL0":"=r"(uvc_last_poll));
    ...
    for(;;) {
        uint64_t uvc_now;
        asm volatile("mrs %0, CNTPCT_EL0":"=r"(uvc_now));
        if ((uvc_now - uvc_last_poll) >= uvc_poll_ticks) {
            uvc_last_poll = uvc_now;
            emu68_usb_audio_housekeeper_poll();
        }
